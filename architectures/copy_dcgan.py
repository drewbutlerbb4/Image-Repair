...
class D(nn.Module):
    ...
    def __init__(self):
        ...
        self.relu1 = nn.LeakyReLU(.2,inplace=True)

        self.conv2 = nn.Conv2d(ndf, 2*ndf, 4, 2,1, bias = False)
        self.batch_norm1 = nn.BatchNorm2d(2*ndf)
        self.relu2 = nn.LeakyReLU(.2,inplace=True)

        self.conv3 = nn.Conv2d(2*ndf, 4*ndf, 4, 2,1, bias = False)
        self.batch_norm2 = nn.BatchNorm2d(4*ndf)
        self.relu3 = nn.LeakyReLU(.2,inplace=True)

        self.conv4 = nn.Conv2d(4*ndf, 8*ndf, 4, 2,1, bias = False)
        self.batch_norm3 = nn.BatchNorm2d(8*ndf)
        self.relu4 = nn.LeakyReLU(.2,inplace=True)

        self.fc1   = nn.Linear(14*14*8*ndf,1024)
        self.relu5 = nn.ReLU()
        self.fc2   = nn.Linear(1024,1)
        self.sig   = nn.Sigmoid()
        ...
        #     nn.LeakyReLU(.2,inplace=True),
        #     nn.Conv2d(8*ndf, 1, 4, 2,0, bias = False),
        #     nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        print(x.shape)
        x = self.relu2(self.batch_norm1(self.conv2(x)))
        print(x.shape)
        x = self.relu3(self.batch_norm2(self.conv3(x)))
        print(x.shape)
        x = self.relu4(self.batch_norm3(self.conv4(x)))
        x=x.view(-1,14*14*8*ndf)
        print(x.shape)
        x=self.relu5(self.fc1(x))
        print(x.shape)
        x=self.sig(self.fc2(x))
...
class G(nn.Module):
    ...
    def __init__(self):
        ...
        self.deconv1 = nn.ConvTranspose2d(3,8*ngf, 4,1,0, bias = False) #(224-2+2) = 224
        self.batch_norm9 = nn.BatchNorm2d(8*ngf)
        self.relu1 = nn.ReLU(True)
        
        self.deconv2 = nn.ConvTranspose2d(8*ngf,4*ngf, 4,2, 1, bias = False) 
        self.batch_norm10 = nn.BatchNorm2d(4*ngf)
        self.relu2 = nn.ReLU(True)

        self.deconv3 = nn.ConvTranspose2d(4*ngf,2 * ngf,4,2, 1, bias = False)
        self.batch_norm11 = nn.BatchNorm2d(2 * ngf)
        self.relu3 = nn.ReLU(True)

        self.deconv4 = nn.ConvTranspose2d(2*ngf,ngf,4,2, 1, bias = False)
        self.batch_norm12 = nn.BatchNorm2d(ngf)
        ...
        # self.net = nn.Sequential(
        #     nn.ConvTranspose2d(3,8*ngf, 4,1,0, bias = False),
    ...
    def forward(self, d2):
        d2 = self.relu1(self.batch_norm9(self.deconv1(d2)))
        d2 = self.relu2(self.batch_norm10(self.deconv2(d2)))
        d2 = self.relu3(self.batch_norm11(self.deconv3(d2)))
        d2 = self.relu4(self.batch_norm12(self.deconv4(d2)))
        d2 = self.tanh(self.deconv5(d2))	
        # print(d2.shape)
        # return self.net(d2)
        return d2
...
for epoch in range(0,100000):
    ...
    for i, data_HR in enumerate(HR_loader,0):
        ...
        for a in range(0,len(real)):
            ...
            w = toImage(real[a]) 
            w = image_editor.sample_damaging(w)
        ...
        errG = (.01*errG_G.to(device)) + errG_L.to(device)
        errG.backward()
        optimizerG.step()
        end=time.time()
    ...
    #Change Path			
    torch.save(netG.state_dict(),'C:/Users/Yami/Desktop/CMSC498V/Image-Repair/models/dcgan_PL_G.pth')
    torch.save(netD.state_dict(),'C:/Users/Yami/Desktop/CMSC498V/Image-Repair/models/dcgan_PL_D.pth')		
